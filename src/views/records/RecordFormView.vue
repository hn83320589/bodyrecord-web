<template>
  <AppLayout>
    <PageHeader title="新增血壓紀錄">
      <RouterLink to="/records" class="text-sm text-gray-500 hover:text-gray-700">返回列表</RouterLink>
    </PageHeader>
    <div class="bg-white rounded-xl shadow p-6 max-w-lg">
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">收縮壓 (mmHg)</label>
            <input v-model.number="form.systolic" type="number" required min="60" max="250"
              class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">舒張壓 (mmHg)</label>
            <input v-model.number="form.diastolic" type="number" required min="40" max="150"
              class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">脈搏 (bpm)</label>
        ...
        <div class="flex gap-3">
          ...
          <RouterLink to="/records" class="px-6 py-2 rounded-lg text-sm border border-gray-300 text-gray-600 hover:bg-gray-50">
            取消
          </RouterLink>
        </div>
      </form>
